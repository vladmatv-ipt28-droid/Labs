...
void FrameProcessor::drawFaces(cv::Mat& frame, const std::vector<Face>& faces) {
    for (const auto& face : faces) {
        cv::rectangle(frame, face.rect, cv::Scalar(0, 255, 0), 2);
        std::string label = "Face: " + std::to_string((int)(face.confidence * 100)) + "%";
        cv::putText(frame, label, cv::Point(face.rect.x, face.rect.y - 5),
